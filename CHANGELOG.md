...
- `check_recall` отвечает письмами Росздравнадзора из официальной еженедельной
  выгрузки контроля качества серий: название препарата, номер серии, тип
  письма, номер и дата.

### Changed

- Поиск, карточка регистрационного удостоверения, статус регистрации и
  предельная цена ЖНВЛП — из официальных суточных выгрузок Минздрава.
- `get_instruction`: текста инструкции в выгрузке ГРЛС нет, ответ
  `ready: false` с причиной.
