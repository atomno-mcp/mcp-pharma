"""CLI-тесты точки входа `atomno-mcp-pharma`."""

from __future__ import annotations

import pytest

from mcp_pharma import __version__
from mcp_pharma.server import (
    _DEFAULT_HTTP_HOST,
    _DEFAULT_HTTP_PORT,
...
class TestHelp:
    def test_dash_dash_help_prints_usage_and_exits_zero(self, capsys) -> None:
        ...
        assert "atomno-mcp-pharma" in out
...
class TestInvalidEnvBailsOutCleanly:
    def test_invalid_env_and_no_cli_flag_exits_two(self, monkeypatch, capsys) -> None:
        monkeypatch.setenv("MCP_PHARMA_LOG_LEVEL", "VERBOSE")
        with pytest.raises(SystemExit) as exc_info:
            main([])
        assert exc_info.value.code == 2
        err = capsys.readouterr().err
        assert "MCP_PHARMA_LOG_LEVEL" in err
